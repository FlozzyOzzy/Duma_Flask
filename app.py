...
def signup():
    ...
    connection.commit()
    return jsonify ({'Success' : 'Thank you for siging up'})
...
def signin():
    ...
    # if condition for zero rows
    if count == 0:
        return jsonify ({"Message": "Login failed"})
    else:
        user = cursor.fetchone()
        return jsonify ({"Message" : "signin successfull", "user": user})
...
def mpesa_payment():
    if request.method == 'POST':
        ...
        consumer_key = "ggKFaIAc2w6mB5RkX9GwHQiugKxWOjpHEl7hSXnpVRx51JdJ"
        consumer_secret = "iAPzNZGOXGzG4gmUXAAMguj1TWWBJeswhvWGcXJIG6S5omYo2zVEaosn33EcreT6"
